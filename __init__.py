...
def replace(match):
    ...
    _field = match.group(2)
    if 'type=' in _field:
        _type = re.search(r'type="(.+?)"', _field).group(1)
    else:
        _type = None
    ...
    if _errors:
        ...
        _content = _content.replace('<strong class="ym-message"></strong>', '')
    return _content.replace('class="row"', 'class="%s"' % _class)

def as_yaml(self):
    "Returns this form rendered as HTML <div>s."
    _search = r'<div class="row"><strong class="ym-message">(.*?)</strong>(.+?)</div>'
    _output = u'<div class="row"><strong class="ym-message">%(errors)s</strong>%(label)s%(field)s%(help_text)s</div>'
    return mark_safe(re.sub(_search, replace, self._html_output(_output, u'%s', '</div>', u'<small>%s</small>', False).replace('\n', '')))
